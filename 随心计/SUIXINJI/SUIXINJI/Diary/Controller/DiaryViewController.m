...
- (void)viewDidLoad {
    ...
    self.index_count = 0;
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"选中跳转日记"] style:UIBarButtonItemStylePlain target:self action:@selector(getTimeData)];
    ...
    //    [JSUserInfo shareManager].classArray = [NSMutableArray array];
...
-(void)customNotiationData{
    ...
    [[[[NSNotificationCenter defaultCenter] rac_addObserverForName:@"addDiaryDataNotication" object:nil]takeUntil:self.rac_willDeallocSignal] subscribeNext:^(NSNotification * _Nullable x) {
        ...
        if ([self.titleStr isEqualToString:addModel.class_name]||[self.titleStr isEqualToString:@"全部"]) {
    ...
    [[[[NSNotificationCenter defaultCenter] rac_addObserverForName:@"deleteDiaryDataNotication" object:nil]takeUntil:self.rac_willDeallocSignal] subscribeNext:^(NSNotification * _Nullable x) {
        @strongify(self);
        //        JSFastLoginModel * addModel = [x.userInfo objectForKey:@"JSFastLoginModel"];
        //        [self.dataArr removeObject:addModel];
...
//    NSMutableAttributedString *numString = [[NSMutableAttributedString alloc] initWithString:cell.timeLabel.text];
//    NSRange ran2 = NSMakeRange(0, 2);
//    [numString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:18] range:ran2];
//    cell.timeLabel.attributedText=numString;
