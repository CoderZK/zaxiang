...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"保存" style:UIBarButtonItemStylePlain target:self action:@selector(saveDiary)];
    ...
    for (NSInteger i=0; i<customArr.count; i++) {
        ...
        if (![model.class_name isEqualToString:@"全部"]) {
...
-(void)saveDiary{
    if (self.model.class_note.length == 0) {
        [SVProgressHUD showInfoWithStatus:@"日记内容不能为空！"];
        return;
    }
    BOOL isClass;
    isClass = YES;
...
//        for (JSImageModel * imageModel in self.realImageModes) {
//            CGSize imaageSize=imageModel.image.size;
//            imaageSize.width*=0.45;
//            imaageSize.height*=0.45;
//            imageModel.image=[self imageWithImage:imageModel.image scaledToSize:imaageSize];
//            NSData*imageData=[NSData dataWithData:UIImageJPEGRepresentation(imageModel.image,0.001)];
////            imageModel.image=[UIImage imageWithData:imageData];
//            [imageArr addObject:imageData];
//        }
//        self.model.class_imageArr = imageArr;
    if (!self.isEditor) {//添加日记时间
...
- (CGFloat)calculateRowHeight:(NSString *)string fontSize:(NSInteger)fontSize{
    ...
    CGRect rect = [string boundingRectWithSize:CGSizeMake(ScreenWidth - 55, 0)/*计算高度要先指定宽度*/ options:NSStringDrawingUsesLineFragmentOrigin |
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        if (self.model.class_note.length==0) {
            cell.textView.PlaceHolder = @"开始写日记";
        } else {
