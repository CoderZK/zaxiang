...
- (void)viewDidLoad {
    ...
    for (JSClassModel * model in [JSUserInfo shareManager].classArray) {
        if (![model.class_name isEqualToString:@"全部"]&&![model.class_name isEqualToString:@"未分类"]) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [nickNameView getCodeBasedInput:^(NSString * _Nonnull password, UIColor * _Nonnull color) {
        ...
        //        if (CGColorEqualToColor(classModel.class_color.CGColor, color.CGColor)||![classModel.class_name isEqualToString:password]) {
        JSClassModel * model = [JSClassModel new];
        model.class_color = color;
        model.class_name = password;
...
-(void)deleteClass:(UIButton*)sender{
    ...
    WS(wSelf);
    [[JSSelectViewTool sharedManager] inpour:[NSMutableArray arrayWithObjects:@"删除分类",@"删除分类和日记", nil] currentIndexd:0 confirmBlock:^(id obj, NSInteger currentIndex) {
